...
module IOStreams
  ...
  module Pgp
    ...
    #   Whether to export as ASCII text instead of binary format
    #   Default: true
    def self.export_secret_key(email:, passphrase:, ascii: true)
      version_check

      armor    = ascii ? '--armor' : nil
      loopback = pgp_version.to_f >= 2.1 ? '--pinentry-mode loopback' : ''
      command  = "echo #{passphrase} | #{executable} #{loopback} --no-tty --passphrase-fd 0 --batch #{armor} --export-secret-keys #{email}"
